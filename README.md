...
Требуются столбцы: **Артикул** и **Остаток** (выбираются вручную в интерфейсе). Обновляется только payload поле `Остаток`, векторы не трогаются. Если артикула нет в коллекции — строка пропускается.
